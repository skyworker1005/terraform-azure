...
# terraform module 
Azure cloud 기반의 KDP(Korea Data Platform) 을 만들려고 한다. 
아래 요구조건을 만족시키는 terraform module 을 만들려고 한다. 
- Databricks 에서 workspace 는 private network에서 접속하고, Cluster는 private network에서 실행되도록 해야한다. 
- Databricks Control plane과의 인터페이스는 private link로 연결해야 한다. 
- 그리고 adsl gen2 도 private network 에서만 접속하도록 해야 한다. 
- adls gen 2는 adf, databricks, synapse analytics 에서만 접속되도록 해야 한다. 
현재 위 요구 조건을 만족시키려고 내가 작성한 코드 구조는 아래와 같다. 
...
module "databricks" {
  ...
  public_subnet_name       = "${var.prefix}-${var.public_subnet_name}"
  private_subnet_name      = "${var.prefix}-${var.private_subnet_name}"
...
data "azurerm_databricks_workspace" "workspace" {
  name                = azurerm_databricks_workspace.dbworkspace.name
  resource_group_name = azurerm_databricks_workspace.dbworkspace.resource_group_name
}
...
resource "azurerm_private_endpoint" "databricks_compute_plane" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = azurerm_databricks_workspace.dbworkspace.id
    subresource_names              = ["databricks_infra"]
    is_manual_connection           = false
  }
}
...
resource "databricks_cluster" "databricks_cluster" {
  ...
  library {
    jar = "dbfs:/mnt/libraries/my-jar.jar"
  }
  ...
  custom_tags = {
    "ClusterName" = "${var.prefix}-${var.environment}-cluster"
    "Environment" = var.environment
  }
  provider = databricks
}
