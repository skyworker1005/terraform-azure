...
- Databricks 에서 workspace 는 Public network에서 접속하고, Cluster는 private network에서 실행되도록 해야한다. 
- Databricks Control plane과의 인터페이스는 private link로 연결해야 한다. 
- 그리고 adsl gent2도 private network 에서만 접속하도록 해야 한다. 
- adls gen 2는 adf, databricks, synapse analytics 에서만 접속되도록 해야 한다. 
현재 위 요구 조건을 만족시키는 코드는 아래와 같다. 
